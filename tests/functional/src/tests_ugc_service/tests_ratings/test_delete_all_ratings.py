...
async def test_delete_all_ratings(
    ...
    for _ in range(3):
        await make_post_request(
            url=f"{test_settings.service_ugc_url}api/v1/ratings/{uuid4()}",
            cookies=cookies,
            headers=headers,
            json={"rating": 8},
        )

    # 6. Удаляем все рейтинги

    body, status, _ = await make_delete_request(
        url=f"{test_settings.service_ugc_url}api/v1/ratings/",
        cookies=cookies,
        headers=headers,
    )
