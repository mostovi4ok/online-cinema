...
def parse_xinfo_stream(response, **options):  # -> dict[Any, Any] | dict[str, str] | dict[str, Any]:
    ...
def parse_xread(response):  # -> list[Any] | list[list[Any | list[Any] | None]]:
    ...
def parse_xread_resp3(response):  # -> dict[Any, Any] | dict[Any, list[list[Any] | None]]:
    ...
def parse_xpending(
    response, **options
):  # -> list[dict[Literal['message_id', 'consumer', 'time_since_delivered', 'times_delivered'], Any]] | dict[str, Any | list[dict[str, Any | int]]]:
    ...
def parse_xpending_range(
    response,
):  # -> list[dict[Literal['message_id', 'consumer', 'time_since_delivered', 'times_delivered'], Any]]:
    ...
...
def parse_zmscore(response, **options):  # -> list[float | None]:
    ...
def parse_slowlog_get(response, **options):  # -> list[dict[str, Any | int]]:
    ...
def parse_stralgo(
    response, **options
):  # -> int | dict[str, list[list[tuple[Any, ...] | int]] | list[list[tuple[Any, ...]]] | int] | str:
    """
...
def parse_cluster_info(response, **options):  # -> dict[str, str]:
    ...
def parse_cluster_nodes(response, **options):  # -> dict[Any, dict[str, Any | bool | list[Any]]]:
    """
    @see: https://redis.io/commands/cluster-nodes  # string / bytes
    @see: https://redis.io/commands/cluster-replicas # list of string / bytes
    """

def parse_geosearch_generic(response, **options):  # -> list[Any] | list[list[float | int | tuple[float, float] | Any]]:
    """
