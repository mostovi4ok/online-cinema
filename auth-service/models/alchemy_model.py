...
class HistoryMixin:
    ...
    def __table_args__(cls) -> tuple[Index]:  # noqa: N805, PLW3201
        return (Index(f"_idx_{cls.__tablename__}", "user_id", "action"),)  # pyright: ignore[reportUnknownMemberType, reportAttributeAccessIssue]


class History(HistoryMixin, Base):
    __tablename__ = "history"
    __table_args__ = ({"postgresql_partition_by": "RANGE (created_at)"},)  # pyright: ignore[reportAssignmentType]
