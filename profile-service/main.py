...
from api.v1 import notifications, profile
...
async def lifespan(_: FastAPI) -> AsyncGenerator[None, Any]:
    ...
    await init_beanie(database=getattr(client, configs.mongo_name), document_models=[ProfileBell])
...
tags_metadata = [
    profile.profile_tags_metadata,
    notifications.notifications_tags_metadata,
]
...
app = FastAPI(
    ...
    description="Сервис пользовательских профилей",
    version="1.0.0",
...
app.include_router(notifications.router, prefix="/api/v1/notifications", dependencies=[Depends(get_jwt_user_global)])
